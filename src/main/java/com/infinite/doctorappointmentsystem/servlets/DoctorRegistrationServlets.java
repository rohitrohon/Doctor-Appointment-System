...
/**
 * Servlet implementation class DoctorRegistrationServlets
 */
public class DoctorRegistrationServlets extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String result1 = service.insertDoctorCred(dc);

		RequestDispatcher requestDispatcher = request.getRequestDispatcher("DoctorLoginPage.jsp");
		if ("success".equals(result1)) {
			String result = service.insertDoctor(doctor);
			if ("success".equals(result)) {
				requestDispatcher.forward(request, response);
			}
		} else {
			requestDispatcher = request.getRequestDispatcher("DoctorRegistrationPage.jsp");
			requestDispatcher.forward(request, response);
		}
	}
